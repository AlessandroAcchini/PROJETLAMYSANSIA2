...
<title>Gestion des Sanctions - Liste des élèves</title>
...
<p class="text-blue-100 text-lg">Gérez les élèves de votre établissement</p>
<div class="flex flex-wrap gap-4 mt-4 pt-2">
<a href="index.php?action=creerEleve"
   class="bg-white text-primary hover:bg-blue-50 font-semibold py-2 px-6 rounded-lg shadow-sm transition flex items-center gap-2">
    <span class="material-icons sm">add</span> Créer un élève
</a>
...
<h3 class="text-lg font-bold text-gray-800 dark:text-white">Liste des élèves</h3>
...
<span class="material-icons sm">add</span> Nouvel élève</a>
</div>
...
<table class="min-w-full divide-y divide-gray-200">
    ...
    <?php if (!empty($eleves)): ?>
        <?php foreach ($eleves as $eleve): ?>
            <tr>
                <td class="px-4 py-2">
                    <?= htmlspecialchars($eleve['nomEleve'], ENT_QUOTES, 'UTF-8') ?>
                    <?= htmlspecialchars($eleve['prenomEleve'], ENT_QUOTES, 'UTF-8') ?>
                </td>
                <td class="px-4 py-2">
                    <?= htmlspecialchars($eleve['nomClasse'], ENT_QUOTES, 'UTF-8') ?>
                </td>
                <td class="px-4 py-2">
                    <?= htmlspecialchars($eleve['niveauClasse'], ENT_QUOTES, 'UTF-8') ?>
                </td>
                <td class="px-4 py-2">
                    <?= htmlspecialchars($eleve['created_at'], ENT_QUOTES, 'UTF-8') ?>
                </td>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
