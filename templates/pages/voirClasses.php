...
<title>Gestion des Sanctions - Liste des classes</title>
...
<p class="text-blue-100 text-lg">Gérez les classes de votre établissement</p>
<div class="flex flex-wrap gap-4 mt-4 pt-2">
<a href="index.php?action=creerClasse"
   class="bg-white text-primary hover:bg-blue-50 font-semibold py-2 px-6 rounded-lg shadow-sm transition flex items-center gap-2">
    <span class="material-icons sm">add</span> Créer une classe
</a>
...
<h3 class="text-lg font-bold text-gray-800 dark:text-white">Liste des classes</h3>
<a href="index.php?action=creerClasse" class="bg-primary hover:bg-blue-700 text-white text-sm font-medium py-2 px-4 rounded shadow transition flex items-center gap-1">
<span class="material-icons sm">add</span> Nouvelle classe</a>
</div>
<div class="overflow-x-auto">
<table class="min-w-full divide-y divide-gray-200">
    <thead class="bg-gray-50">
        <tr>
            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom de la classe</th>
            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Niveau</th>
            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CRÉÉE LE </th>
        </tr>
    </thead>
    ...
    <?php if (!empty($classes)): ?>
        <?php foreach ($classes as $classe): ?>
            <tr>
                ...
                <td class="px-4 py-2">
                    <?= htmlspecialchars($classe['niveauClasse'], ENT_QUOTES, 'UTF-8') ?>
                </td>
                <td class="px-4 py-2">
                    <?= htmlspecialchars($classe['created_at'], ENT_QUOTES, 'UTF-8') ?>
                </td>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
